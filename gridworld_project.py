...
#This code is an adaptation of that of Matteo Pirotta, MVA Reinforcement Learning professor

#Variation of gridworld :
#it is now possible to do a step from a state with a number
#every state is supposed to have a number
#terminal states will have a 0
...
class GridWorld:
    ...
    def step(self, state, action, project = False):
        """
        Args:
        ...
        if False:
            if project == False:
                ...
                return state, self.grid[r][c], True
        else:
    ...
    def compute_available_actions(self):
        ...
        for i in range(self.n_rows):
            for j in range(self.n_cols):
                #deleted for the projects as every state has a cost
...
GridWorld2 = GridWorld(gamma=1, grid=grid2)

def generate_a_grid(sizegrid, maxval):
    grid = (np.random.randint(maxval-1,size = (sizegrid,sizegrid))+np.ones((sizegrid,sizegrid))).tolist()
    grid[sizegrid//2][1]='x'
    grid[sizegrid//2][-2]='x'
    grid[1][sizegrid//2]=0
    grid[-2][sizegrid//2]=0
...
def generate_a_grid_withwalls(sizegrid, maxval):
    grid = (np.random.randint(maxval-1,size = (sizegrid,sizegrid))+np.ones((sizegrid,sizegrid))).tolist()
    for i in range(sizegrid//4,sizegrid-1):
        grid[sizegrid//4][i]='x'
    for i in range(1,3*sizegrid//4):
        grid[3*sizegrid//4][i]='x'
    grid[1][sizegrid//2]=0
    grid[-2][sizegrid//2]=0
    return grid

#GridWorld2 = GridWorld(gamma=0.95, grid=generate_a_grid(7,10))